...
/**
 * Съёмная вкладка может быть отделена от tab pane и показана в отдельном окне.
 * Съёные вкладовки могут быть добавлены к дюбому TabPane, но для более корректной
 * работы все вкладки должны быть типа DraggableTabs!
 * <p>
 *
...
public class DraggableTab extends Tab {
    ...
    private boolean detachable; // возможность отрываться от tab pane
    ...
    public DraggableTab(String text) {
        ...
        nameLabel.setOnMouseDragged(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent t) {
                double width = nameLabel.getWidth() + 40;
                double height = nameLabel.getHeight() + 10;

                dragStage.setWidth(width);
                dragStage.setHeight(height);
                ...
                try {
                    data = getInsertData(screenPoint);
                } catch (Exception e) {
                ...
                if (data == null || data.getInsertPane().getTabs().isEmpty()) {
                    ...
                    if (end) {
                        markerStage.setX(rect.getMaxX() + 29);
                    } else {
    ...
    private Rectangle2D getAbsoluteRect(Control node) {
        return new Rectangle2D(
                node.localToScene(node.getLayoutBounds().getMinX(),
                        node.getLayoutBounds().getMinY()).getX() + node.getScene().getWindow().getX(),
                node.localToScene(node.getLayoutBounds().getMinX(),
                        node.getLayoutBounds().getMinY()).getY() + node.getScene().getWindow().getY(),
                node.getWidth(),
                node.getHeight());
    }
